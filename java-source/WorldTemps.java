...
public class WorldTemps 
{
	...
	public static void importFile(LinkedList ll, String unused) {
		try {
			long count = 0;
			String csvUrl = "https://onlqrhfjvwm2vstl.public.blob.vercel-storage.com/city_temperature.csv";

			System.out.println("Loading CSV from remote URL: " + csvUrl);

			BufferedReader br = new BufferedReader(
				new InputStreamReader(new URL(csvUrl).openStream())
			);

			String newLine = br.readLine(); // skip header
			newLine = br.readLine();
			String parseBy = ",";
			...
			System.out.printf("File loaded: %d lines\n", count);
			br.close();
		}
		catch (Exception e) {
